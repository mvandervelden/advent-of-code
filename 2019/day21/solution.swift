...
class Solution {
  ...
  private func solveOne(file: File) -> String {
    ...
    let line = lines[0]
    code = line.indexDict
    let springcode = File(filename: "example.1.txt").string
    runInput(instructions: springcode)
    return "finished"
  }
  ...
  private func runInput(instructions: String) {
    let group = DispatchGroup()
    program = IntCode(code: code)
    program.inputs = instructions.map { Int($0.asciiValue!) }
...
extension Solution: IntCodeOutputReceiver {
  func handleValue(_ value: Int) {
    outputString.append(Character(Unicode.Scalar(value)!))
    // TODO parse output into grid?
  }
}
